	...
	stages {
		...
		stage('Build Docker Image')
				...
				sh 'sudo chmod 666 /var/run/docker.sock'
                                sh 'docker build -t asteward/csc324flaskapp:$VERSION .'
                                sh 'docker build -t asteward/csc324flaskapp:latest .'
                        }
                }
		...
		stage('Push Image to Dockerhub') {
			...
			steps {
				sh 'docker push asteward/csc324flaskapp:$VERSION'
				sh 'docker push asteward/csc324flaskapp:latest'
			}
		}
